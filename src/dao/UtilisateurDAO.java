...
public class UtilisateurDAO extends DAO<Utilisateur>{
    ...
    public List<Utilisateur> liste() {
        ...
        try {
         ...
         public boolean connexion(String login, String password, String profil) {
       boolean v = false;
        try {
           
            ResultSet result = this.connect.createStatement().executeQuery("select * from utilisateur where login='"+login+"' and password='"+password+"'and profil='"+profil+"'");
            if(result.next()){
               v=true;
            }
        } catch (SQLException ex) {
